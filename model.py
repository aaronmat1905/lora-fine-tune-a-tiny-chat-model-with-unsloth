    ...
    return [
        {
            ...
            'instruction':'Give a random city from South India',
            'response': 'Bangalore'
        }, {
            'instruction': 'How many states are there in India?', 
            'response': '29 states'
        }, {
            'instruction': 'What is the national bird of India?', 
            'response': 'Peacock'
        }, {
            'instruction': 'Where is New Delhi Located?',
            'response': 'India'
        }
    ]
...
def format_all_examples(examples):
    ...
    return [
        format_instruction_example(i) for i in examples
    ]
...
def generate_reply(model, tokenizer, prompt, max_new_tokens=32):
    """Tokenize `prompt`, greedy-decode a reply, and return just the new text."""
    model.eval()  # disable dropout etc. so greedy decoding is truly deterministic
